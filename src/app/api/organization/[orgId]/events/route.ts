...
export async function GET(request: Request, { params }: { params: Promise<{ orgId: string }> }) {
  try {
    ...
    console.log(`GET /api/organization/${orgId}/events called, type=${type}`);
    ...
    const events = await prisma.clubEvent.findMany({
      ...
      include: {
        registrations: {
          include: {
            member: { include: { player: { include: { user: true } } } },
          },
        },
        bracket: true,
        matches: true,
    ...
    console.log(`Found ${events.length} events for orgId=${orgId}`);
