...
export async function PATCH(
  ...
  try {
    ...
    if (!orgId || orgId === 'undefined') {
      console.error(`[Reject Task] Invalid orgId: ${orgId}, taskId: ${taskId}`);
      return new Response(JSON.stringify({ error: 'Invalid organization ID' }), { status: 400 });
    }

    if (!reason) {
      return new Response(JSON.stringify({ error: 'Rejection reason required' }), { status: 400 });
    }
    ...
    if (typedTask) {
      ...
      if (typedTask.organizationId !== orgId) {
        console.error(`[Reject Task] Org mismatch - Task org: ${typedTask.organizationId}, URL org: ${orgId}`);
        return new Response(JSON.stringify({ error: 'Unauthorized - Organization mismatch' }), { status: 403 });
      }

      if (typedTask.status !== TaskStatus.ASSIGNED && typedTask.status !== TaskStatus.ACCEPTED) {
        return new Response(
          JSON.stringify({ error: `Cannot reject task with status: ${typedTask.status}` }),
          { status: 400 }
        );
      }

      // Update typed task to FAILED (rejected)
    ...
    return new Response(JSON.stringify({ error: 'Failed to reject task' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
