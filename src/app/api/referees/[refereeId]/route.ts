...
export async function GET(_req: NextRequest, context: { params: Promise<{ refereeId: string }> }) {
  try {
...
export async function PUT(req: NextRequest, context: { params: Promise<{ refereeId: string }> }) {
  try {
    const { refereeId } = await context.params;
    ...
    if (!payload || payload.role !== 'referee' || payload.id !== refereeId) {
