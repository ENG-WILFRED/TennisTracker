...
export async function POST(request: Request) {
  try {
    ...
    console.log('PayPal route received:', {
    ...
    if (errors.length > 0) {
      ...
      console.error('PayPal validation failed:', errorMsg);
      return new Response(
    ...
    const result = await processPayPalPayment(amount, currency, userId, eventId, bookingType, metadata);
