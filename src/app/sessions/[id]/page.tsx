...
export default function SessionDetailPage() {
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        ...
        // Initialize reschedule form with current session data
  ...
  const handleUpdateSession = async () => {
    router.push(`/dashboard?editSessionId=${sessionId}`);
  };
        ...
        <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <button
              ...
              disabled={isPast || isActionLoading}
              style={{
                padding: '12px 16px',
                background: isPast ? G.muted : G.blue,
                color: '#fff',
                border: 'none',
                borderRadius: 8,
                fontSize: 12,
            ...
            >
              📅 {showRescheduleForm ? 'Hide' : 'Reschedule'}
            </button>

            <button
              onClick={() => setShowRemindModal(!showRemindModal)}
              disabled={isPast || isActionLoading}
              style={{
                padding: '12px 16px',
                background: isPast ? G.muted : G.bright,
                color: '#fff',
                border: 'none',
                borderRadius: 8,
                fontSize: 12,
            ...
            <button
              ...
              style={{
                padding: '12px 16px',
                background: isPast ? G.muted : G.red,
                color: '#fff',
                border: 'none',
                borderRadius: 8,
                fontSize: 12,
      ...
      {showRescheduleForm && !isPast && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ margin: '0 0 16px 0', color: G.lime2, fontSize: 16, fontWeight: 700 }}>
              📅 Reschedule Session
            </h2>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 10, color: G.muted2, display: 'block', marginBottom: 6, fontWeight: 700 }}>NEW DATE</label>
                <input
                  type="date"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 10, color: G.muted2, display: 'block', marginBottom: 6, fontWeight: 700 }}>START TIME</label>
                  <input
                    type="time"
                ...
                <div>
                  <label style={{ fontSize: 10, color: G.muted2, display: 'block', marginBottom: 6, fontWeight: 700 }}>END TIME</label>
                  <input
                    type="time"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              >
                {isActionLoading ? '⏳ Updating...' : '✓ Reschedule'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showRemindModal && !isPast && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ margin: '0 0 12px 0', color: G.lime2, fontSize: 16, fontWeight: 700 }}>
              Send Reminders?
            </h2>
            <p style={{ color: G.muted, fontSize: 12, marginBottom: 20 }}>
              Send reminder notifications to all {session.players?.length || 0} enrolled players about this session.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              >
                {isActionLoading ? '⏳ Sending...' : '✓ Send Reminders'}
              </button>
            </div>
          </div>
        </div>
