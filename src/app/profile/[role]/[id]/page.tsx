    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Segoe UI', system-ui, sans-serif", background: G.dark, color: G.text, overflow: 'hidden' }}>
      ...
      <aside style={{ width: 188, background: G.sidebar, borderRight: `1px solid ${G.cardBorder}`, display: 'flex', flexDirection: 'column', flexShrink: 0, overflowY: 'auto' }}>
        ...
        <div style={{ margin: '0 10px 12px', background: G.mid, borderRadius: 8, padding: '10px 12px' }}>
          <div style={{ fontSize: 9, color: G.accent, fontWeight: 700, letterSpacing: 1 }}>PROFILE</div>
          <div id="sidebarName" style={{ fontSize: 12, fontWeight: 700, marginTop: 4 }}>Loading...</div>
          <div id="sidebarEmail" style={{ fontSize: 10, color: G.muted, marginTop: 2 }}>-</div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {/* Profile View Component - Reusable */}
        <ProfileView isEmbedded={false} canEdit={false} />
      </main>
    </div>
  );
}
