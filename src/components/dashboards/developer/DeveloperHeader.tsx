...
export function DeveloperHeader({
  ...
  return (
    <header className="mb-6 grid gap-4">
      <div className="flex flex-col gap-4 rounded-[32px] border border-slate-800/60 bg-slate-950/70 p-5 shadow-[0_16px_80px_rgba(15,23,42,0.35)] sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-start sm:justify-end">
          <div className="rounded-3xl border border-slate-700/60 bg-slate-900/70 px-4 py-3 text-xs text-slate-300 shadow-sm">
            <p className="text-slate-400">Connected as</p>
            <p className="mt-1 font-semibold text-white">{user?.username || 'Developer'}</p>
          </div>
          <button
            onClick={onLogout}
            className="rounded-3xl border border-slate-700/60 bg-slate-900/70 px-4 py-3 text-xs font-semibold text-cyan-200 transition hover:bg-slate-800/90"
          >
            Sign out
          </button>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-3xl border border-slate-800/60 bg-slate-900/70 p-4">
