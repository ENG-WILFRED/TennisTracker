    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 11, height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button style={{ background: G.lime, color: '#0a180a', border: 'none', borderRadius: 7, padding: '7px 12px', fontWeight: 800, fontSize: 10.5, cursor: 'pointer' }}>
            + New Session
          </button>
        </div>
      </div>
              ...
              {selectedDay && (
                <button
                  onClick={() => {
                    ...
                    // Navigate to create session with date pre-filled
                    window.location.href = `/dashboard/coach/${coachId}?section=Sessions&date=${dateStr}`;
                  }}
                  style={{
                    background: G.lime,
                    color: '#0a180a',
            ...
            {selectedDaySessions.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7, flex: 1, minHeight: 0, overflowY: 'auto' }}>
                {selectedDaySessions.map(s => (
                  <div key={s.id} style={{ background: G.card2, border: `1px solid ${G.border}`, borderRadius: 8, padding: 10, borderLeft: `3px solid ${sessionColors[s.sessionType] || G.lime}` }}>
                    <div style={{ fontSize: 11.5, fontWeight: 700, marginBottom: 3 }}>{s.title}</div>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      <Tag color={sessionColors[s.sessionType]}>{s.sessionType}</Tag>
