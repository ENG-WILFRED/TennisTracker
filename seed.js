...
async function main() {
            ...
            for (const player of players) {
                await prisma.coachReview.create({
                    data: {
                        ...
                        reviewText: ['Great coach!', 'Very professional', 'Learned a lot', 'Highly recommended'][Math.floor(Math.random() * 4)],
                    }
                }).catch(() => { });
            }
        }
    ...
    if (allPlayers.length > 0 && allBadges.length > 0) {
        ...
        if (pbData.length > 0)
            await prisma.playerBadge.createMany({ data: pbData, skipDuplicates: true });
    ...
    try {
        ...
        const clubMember1 = await prisma.clubMember.upsert({
            where: { organizationId_playerId: { organizationId: org1.id, playerId: players[0].id } },
            update: {},
            create: {
                organizationId: org1.id,
        ...
        const clubMember2 = await prisma.clubMember.upsert({
            where: { organizationId_playerId: { organizationId: org1.id, playerId: players[1].id } },
            update: {},
            create: {
                organizationId: org1.id,
